...
public final class Operator extends Enum<Operator> {
    ...
    // 2. 모든 상수를 보관하는 배열
    private static final Operator[] VALUES = { ADD, SUBTRACT, MULTIPLY, DIVIDE };

    // 3. private 생성자 (외부에서 new 불가)
    private Operator(String name, int ordinal) {
        super(name, ordinal);
    }

    // 4. values(), valueOf() 메서드 자동 생성 - 원본 배열을 보호하고 값을 찾을 수 있게 도와주는 메서드
    public static Operator[] values() { return VALUES.clone(); }
